...
<template>
    <div class="p-6 md:p-10 bg-background min-h-screen text-foreground">
        ...
        <div class="max-w-3xl mx-auto relative">
            ...
            <div class="mb-8">
                <h1 class="text-3xl font-bold tracking-tight mb-2">Publicar Nuevo Material</h1>
                <p class="text-muted-foreground">Completa los datos del material que quieres ofrecer.</p>
            </div>
            ...
            <Card class="bg-card border-border shadow-none overflow-hidden">
                <form @submit.prevent="submit">
                    <CardHeader class="border-b border-border">
                        ...
                        <CardDescription>La empresa se asignará automáticamente.</CardDescription>
                    </CardHeader>
                    ...
                    <CardContent class="p-6 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <Input id="nombre" v-model="form.nombre" placeholder="Ej: Botellas PET" 
                            ...
                            <div>
                                ...
                                <select id="idcategorias" v-model="form.idcategorias" 
                                    class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground focus-visible:ring-primary">
                        ...
                        <div>
                            ...
                            <textarea id="descripcion" v-model="form.descripcion" rows="3" 
                                class="flex w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground focus-visible:ring-primary"
                                placeholder="Describe el estado y características del material..."></textarea>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                <Label for="cantidad" class="text-muted-foreground mb-2 block">Cantidad</Label>
                                <Input id="cantidad" type="cantidad" v-model="form.cantidad" placeholder="0.00" 
                                    class="bg-background border-border text-foreground focus-visible:ring-primary" />
                            </div>
                            <div>
                                <Label for="unidadMedida" class="text-muted-foreground mb-2 block">Unidad de Medida</Label>
                                <select id="unidadMedida" v-model="form.unidadMedida" 
                                    class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground focus-visible:ring-primary">
                            ...
                            <div>
                                ...
                                <select id="frecuencia" v-model="form.frecuencia" 
                                    class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground focus-visible:ring-primary">
                                    <option value="" disabled>Seleccione...</option>
                        ...
                        <div>
                            <Label for="urlImagen" class="text-muted-foreground mb-2 block">Imagen del Material</Label>
                            <div class="flex items-center gap-4">
                                ...
                                <span v-if="form.urlImagen" class="text-sm text-muted-foreground truncate">
                                    {{ form.urlImagen.name }}
                                </span>
