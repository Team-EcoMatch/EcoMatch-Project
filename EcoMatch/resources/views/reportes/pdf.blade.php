...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .meta { display: flex; justify-content: space-between; margin-bottom: 18px; font-size: 9px; color: #555; }
        .section { margin-bottom: 20px; }
        .section h2 { font-size: 13px; color: #1E40AF; border-bottom: 1px solid #BFDBFE; padding-bottom: 4px; margin-bottom: 8px; }
        table { width: 100%; border-collapse: collapse; margin-bottom: 8px; }
        th, td { padding: 5px 7px; text-align: left; border-bottom: 1px solid #e5e7eb; font-size: 9px; }
        th { background: #EFF6FF; font-weight: bold; color: #1E40AF; }
        .stats { display: flex; gap: 8px; margin-bottom: 12px; }
        .stat-box { border: 1px solid #BFDBFE; padding: 8px; flex: 1; background: #F8FAFF; }
        .stat-label { font-size: 8px; color: #666; text-transform: uppercase; }
        .stat-value { font-size: 16px; font-weight: bold; color: #1E40AF; margin-top: 2px; }
        .footer { position: fixed; bottom: 0; left: 0; right: 0; text-align: center; font-size: 8px; color: #999; border-top: 1px solid #e5e7eb; padding-top: 6px; }
    </style>
</head>
<body>
    <div class="header">
    ...
    <div class="section">
        ...
        <table>
            <thead>
                <tr><th>Unidad</th><th>Cantidad Total</th><th>N° Intercambios</th></tr>
            </thead>
            <tbody>
    ...
    <div class="section">
        ...
        <table>
            <thead><tr><th>#</th><th>Material</th><th>Publicaciones</th></tr></thead>
            <tbody>
                @foreach($topMateriales as $i => $mat)
                <tr><td>{{ $i + 1 }}</td><td>{{ $mat->nombre }}</td><td>{{ $mat->total }}</td></tr>
                @endforeach
            </tbody>
        </table>
    </div>
    ...
    <div class="section">
        ...
        <table>
            <thead><tr><th>#</th><th>Empresa</th><th>Publicaciones</th></tr></thead>
            <tbody>
                @foreach($topEmpresas as $i => $emp)
                <tr><td>{{ $i + 1 }}</td><td>{{ $emp->nombreEmpresa }}</td><td>{{ $emp->publicaciones_count }}</td></tr>
                @endforeach
            </tbody>
        </table>
    </div>
    ...
    <div class="section">
        ...
        <table>
            ...
            <tbody>
                ...
                <tr><td>{{ $cat->nombre }}</td><td>{{ $cat->total }}</td></tr>
                @endforeach
            </tbody>
        </table>
    </div>
